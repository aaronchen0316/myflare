...
class Parameters:
    ...
    logger = set_logger("Parameters", stream=True, fileout_name=None, verbose="info")
        ...
        for key in keys:
            for original in replace_list:
                if original in key and replace_list[original] not in key:
                    ...
                    DeprecationWarning("{key} is being replaced with {newkey}")
        ...
        if set(kernels) != set(param_dict.get("kernels", [])):
            ...
            for k in Parameters.all_kernel_types:
                if k in kernels:
                    ...
                    Warning(
                        "inconsistency between input kernel and kernel list"
                        "stored in hyps_mask"
                    )
    ...
    def check_instantiation(hyps, cutoffs, kernels, param_dict):
        ...
        if "map" in param_dict:
            ...
            if (len(param_dict["original_hyps"]) - 1) not in param_dict["map"]:
                assert (
                    param_dict["train_noise"] is False
                ), "train_noise should be False when noise is not in hyps"

            assert len(param_dict["map"]) == len(
                hyps
    ...
    @staticmethod
    def get_component_hyps(
        param_dict, kernel_name, hyps=None, constraint=False, noise=False
    ):
        """
...
<<<<<<< HEAD
            specie_mask = param_dict["specie_mask"]
=======
